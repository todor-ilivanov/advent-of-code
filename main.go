package main

import (
	"fmt"

	"advent/day01"
	"advent/day02"
	"advent/day03"
	"advent/day04"
	"advent/day05"
	"advent/day06"
	"advent/day07"
	"advent/day08"
	"advent/day09"
	"advent/day10"
	"advent/day11"
	"advent/day12"
	"advent/day13"
	"advent/day14"
	"advent/day15"
	"advent/day17"
	"advent/day18"
	"advent/day21"
	"advent/day23"
)

func main() {
	// solveDay01()
	// solveDay02()
	// solveDay03()
	// solveDay04()
	// solveDay05()
	// solveDay06()
	// solveDay07()
	// solveDay08()
	// solveDay09()
	// solveDay10()
	// solveDay11()
	// solveDay12()
	// solveDay13()
	// solveDay14()
	// solveDay15()
	// solveDay17()
	// solveDay18()
	// solveDay21()
	solveDay23()
}

func solveDay01() {
	fmt.Println("----Day 01----")
	part1 := day01.SolvePart1()
	part2 := day01.SolvePart2()
	fmt.Println(part1)
	fmt.Println(part2)
}

func solveDay02() {
	fmt.Println("----Day 02----")
	part1 := day02.SolvePart1()
	part2 := day02.SolvePart2()
	fmt.Println(part1)
	fmt.Println(part2)
}

func solveDay03() {
	fmt.Println("----Day 03----")
	part1 := day03.SolvePart1()
	part2 := day03.SolvePart2()
	fmt.Println(part1)
	fmt.Println(part2)
}

func solveDay04() {
	fmt.Println("----Day 04----")
	part1, part2 := day04.Solve()
	fmt.Println(part1)
	fmt.Println(part2)
}

func solveDay05() {
	fmt.Println("----Day 05----")
	part1 := day05.SolvePart1()
	part2 := day05.SolvePart2()
	fmt.Println(part1)
	fmt.Println(part2)
}

func solveDay06() {
	fmt.Println("----Day 06----")
	part1 := day06.SolveForWindowSize(4)
	part2 := day06.SolveForWindowSize(14)
	fmt.Println(part1)
	fmt.Println(part2)
}

func solveDay07() {
	fmt.Println("----Day 07----")
	part1, part2 := day07.Solve()
	fmt.Println(part1)
	fmt.Println(part2)
}

func solveDay08() {
	fmt.Println("----Day 08----")
	part1 := day08.SolvePart1()
	fmt.Println(part1)
	part2 := day08.SolvePart2()
	fmt.Println(part2)
}

func solveDay09() {
	fmt.Println("----Day 09----")
	part1 := day09.SolvePart1()
	fmt.Println(part1)
	part2 := day09.SolvePart2(10)
	fmt.Println(part2)
}

func solveDay10() {
	fmt.Println("----Day 10----")
	res := day10.Solve()
	fmt.Println(res)
}

func solveDay11() {
	fmt.Println("----Day 11----")
	part1 := day11.Solve(20, 3)
	part2 := day11.Solve(10000, 1)
	fmt.Println(part1)
	fmt.Println(part2)
}

func solveDay12() {
	fmt.Println("----Day 12----")
	part1, part2 := day12.Solve()
	fmt.Println(part1)
	fmt.Println(part2)
}

func solveDay13() {
	fmt.Println("----Day 13----")
	part1, part2 := day13.Solve()
	fmt.Println(part1)
	fmt.Println(part2)
}

func solveDay14() {
	fmt.Println("----Day 14----")
	part1, part2 := day14.Solve()
	fmt.Println(part1)
	fmt.Println(part2)
}

func solveDay15() {
	fmt.Println("----Day 15----")
	part1, part2 := day15.Solve()
	fmt.Println(part1)
	fmt.Println(part2)
}

func solveDay17() {
	fmt.Println("----Day 17----")
	part1, part2 := day17.Solve()
	fmt.Println(part1)
	fmt.Println(part2)
}

func solveDay18() {
	fmt.Println("----Day 18----")
	part1, part2 := day18.Solve()
	fmt.Println(part1)
	fmt.Println(part2)
}

func solveDay21() {
	fmt.Println("----Day 21----")
	part1, part2 := day21.Solve()
	fmt.Println(part1)
	fmt.Println(part2)
}

func solveDay23() {
	fmt.Println("----Day 23----")
	part1, part2 := day23.Solve(10)
	fmt.Println(part1)
	fmt.Println(part2)
}
